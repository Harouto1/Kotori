...
module.exports = new Event("messageCreate", async (bot, message) => {
    ...
    db.query(`SELECT * FROM serveur WHERE guildID = ${message.guild.id}`, async (err, req) => {
        ...
        if(req.length < 1) {
            ...
            let sql = `INSERT INTO serveur (guildID, prefix, raid) VALUES (${message.guild.id}, 'k!', 'off')`
            db.query(sql, function(err) {
                if(err) throw err;
            })

            return message.reply("Wait for the bot to register your server !")
        }
            ...
            if(req.length < 1) {
                ...
                if(!message.content.startsWith(prefix)) {
                    ...
                    if(parseInt(req[0].xp) < 0) {
                        ...
                        message.channel.send(`Dommage ${message.author}, you level down \`${parseInt(req[0].level) - 1}\``)
                    }
                }
            }
        })
        ...
        if(!commandFile) return message.reply(`This command does not exist !`)
        ...
        if(cooldown.has(message.author.id)) {
            ...
            if(time < timeRestant) {
                ...
                return message.reply(`You must wait ` + `\`${(Math.round(timeLeft / (1000 * 60 * 60 * 24) % 30))}\`` + ` Day(s) ` + `\`${(Math.round(timeLeft / (1000 * 60 * 60)))}\`` + ` Hour(s) ` + `\`${(Math.round(timeLeft / (1000 * 60) % 60))}\`` + ` minute(s) ` + `\`${(Math.round(timeLeft / 1000 % 60))}\`` + ` second(s) to run this command !`)
            }
        }
        ...
        if(commandFile.permission === "Développeur" && message.author.id !== "244950891298422786") return message.reply("You do not have the required permission to run this command !")
        if(commandFile.permission !== "Aucune" && commandFile.permission !== "Développeur" && !message.member.permissions.has(new Discord.Permissions(commandFile.permission))) return message.reply("You do not have the required permission to run this command !")
